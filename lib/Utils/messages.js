...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        const coverStream = (await (0, messages_media_1.getStream)(cover)).stream;
        const coverChunks = [];
        for await (const chunk of coverStream) coverChunks.push(chunk);
        const coverBuffer = Buffer.concat(coverChunks);
        const stickerPackUpload = await (0, messages_media_1.encryptedStream)(zipBuffer, 'sticker-pack', {
            logger: options.logger,
            opts: options.options
        });
        const _messages_media_1 = require('./messages-media.js');
        const { getMediaKeys: _getMediaKeys } = _messages_media_1;
        ...
        try {
            const _messages_media_1 = require('./messages-media.js');
            const { extractImageThumb } = _messages_media_1;
        ...
        m.stickerPackMessage.contextInfo = {
            ...(message.contextInfo || {}),
            ...('mentions' in message && message.mentions ? { mentionedJid: message.mentions } : {})
        };
    }
