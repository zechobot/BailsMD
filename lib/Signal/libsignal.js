...
const jidToSignalProtocolAddress = (jid) => {
    const { user, device, domainType } = (0, WABinary_1.jidDecode)(jid);
...
function signalStorage({ creds, keys }, lidMapping) {
    const resolveLIDSignalAddress = async (id) => {
        if (id.includes('.')) {
            ...
            // WAJIDDomains.LID = 1, HOSTED_LID = 3
            if (domainType === 1 || domainType === 3)
                return id;

            // WAJIDDomains.HOSTED = 2
            const pnJid = `${user}${device !== '0' ? `:${device}` : ''}@${domainType === 2 ? 'hosted' : 's.whatsapp.net'}`;
